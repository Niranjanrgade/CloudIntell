...
This chapter surveys the theoretical and technical foundations that inform the system design. It covers multi-agent coordination, retrieval-augmented generation, agentic reasoning patterns, and the tools selected for implementation, before identifying the research gaps that motivate this work.
...
The Contract Net Protocol \cite{smith1980contract} remains a seminal contribution by formalizing decentralized task negotiation, allocation, and coordination among agents via bidding and task contracting, establishing a formal basis for distributed problem-solving architectures. That negotiation pattern is reflected in this thesis's architect supervisor and domain-specific generator/validator agents, which assign and accept tasks through a comparable delegation mechanism.
...
In this framework, Docling serves as the document parsing step that converts raw source files into clean, semantically meaningful fragments suitable for downstream embedding and indexing \cite{docling2024web}.
...
The integration of ChromaDB in the knowledge retrieval pipeline is central to retrieval performance. Unlike keyword search, cosine similarity over embeddings connects semantically related but differently phrased passages---an important property when cloud service documentation uses varied naming conventions for equivalent concepts. ChromaDB therefore serves as the retrieval layer that grounds validator agent assessments in official documentation.
...
Traditional evaluation metrics for natural language generation, such as BERTScore or METEOR, measure surface-level token overlap and struggle when multiple valid architectural solutions exist for the same requirement. 
Zheng et al.~\cite{zheng2024judging} formalise the LLM-as-a-judge paradigm, demonstrating that a strong frontier model can reliably score open-ended responses on expert criteria, achieving high agreement with human annotators when given carefully structured rubrics. Crucially, they identify that the choice of judge model matters: a judge drawn from the same model family as the system under test risks self-serving bias. This finding informed the evaluation design, where an independent judge model scores outputs across six technical dimensions (completeness, accuracy, security, scalability, best practices adherence, and specificity) on a 1--10 scale.
...
Recent proprietary solutions including Amazon Q \cite{aws2023q} and Microsoft Copilot for Azure \cite{microsoft2024copilot} exemplify AI integration in cloud ecosystems. Despite their capabilities, proprietary tools are confined to single cloud ecosystems and lack end-to-end multi-cloud solutions or architectural debate mechanisms---a gap this research addresses.

\section{Identifying the Research Gap}

The literature synthesis points to three unmet needs:
...
This research addresses these gaps by building a multi-provider framework that pairs cyclical validation with two-stage reranked retrieval and cross-provider architectural comparison, making informed cloud architecture decisions more tractable.